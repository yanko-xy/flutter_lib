...
class CustomSuperNode extends ElementNode {
  final String text;
      ...
      if (element is ConcreteElementNode && element.children[0] is LinkNode) {
        ...
        if (dataMap != null) {
          for (var key in dataMap!.keys) {
            if (linkNode.attributes.containsKey(key)) {
              late TextStyle value;
              if (dataMap![key] is Map) {
                value = dataMap![key][linkNode.attributes[key]];
              } else {
                value = dataMap![key];
              }
              linkStyle = linkStyle.merge(value);
            }
          }
        }
